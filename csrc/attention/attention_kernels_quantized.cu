...
template <typename scalar_t, typename cache_t, int HEAD_SIZE, int BLOCK_SIZE,
          int NUM_THREADS>
__global__ void single_query_cached_kv_attention_quantized_kernel(
    ...
    const scalar_t
        *__restrict__ quant_params, // [num_blocks, num_kv_heads, block_size, 4]
...
#pragma unroll
      for (int j = 0; j < NUM_VECS_PER_THREAD; j++) {
        ...
        Vec_quant k_vec_quant = *reinterpret_cast<const Vec_quant *>(
...
#define LAUNCH_ATTENTION_QUANTIZED_KERNEL(T, HEAD_SIZE, BLOCK_SIZE,            \
                                          NUM_THREADS)                         \
  cudaFuncSetAttribute(                                                        \
      vllm::single_query_cached_kv_attention_quantized_kernel<                 \
          T, int8_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS>,                      \
      cudaFuncAttributeMaxDynamicSharedMemorySize, shared_mem_size);           \
  vllm::single_query_cached_kv_attention_quantized_kernel<                     \
      T, int8_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS>                           \
      <<<grid, block, shared_mem_size, stream>>>(                              \
          out_ptr, query_ptr, key_cache_ptr, value_cache_ptr,                  \
          quant_params_ptr, head_mapping_ptr, scale, block_tables_ptr,         \
          context_lens_ptr, max_num_blocks_per_seq, alibi_slopes_ptr,          \
          q_stride, kv_block_stride, kv_head_stride, num_kv_heads);
...
void single_query_cached_kv_attention_quantized_launcher(
    ...
    torch::Tensor &value_cache, torch::Tensor &quant_params,
    torch::Tensor &head_mapping, float scale, torch::Tensor &block_tables,
  ...
  int8_t *key_cache_ptr = reinterpret_cast<int8_t *>(key_cache.data_ptr());
  int8_t *value_cache_ptr = reinterpret_cast<int8_t *>(value_cache.data_ptr());
  T *quant_params_ptr = reinterpret_cast<T *>(quant_params.data_ptr());
  ...
  // 32, 160, 192.
  // case 32:
  //   LAUNCH_ATTENTION_QUANTIZED_KERNEL(T, 32, BLOCK_SIZE, NUM_THREADS);
  //   break;
  case 64:
  ...
  case 128:
    ...
    break;
  // case 160:
  //   LAUNCH_ATTENTION_QUANTIZED_KERNEL(T, 160, BLOCK_SIZE, NUM_THREADS);
  //   break;
  // case 192:
  //   LAUNCH_ATTENTION_QUANTIZED_KERNEL(T, 192, BLOCK_SIZE, NUM_THREADS);
  //   break;
  case 256:
...
#define CALL_KERNEL_QUANTIZED_LAUNCHER(T, BLOCK_SIZE)                          \
  single_query_cached_kv_attention_quantized_launcher<T, BLOCK_SIZE>(          \
      out, query, key_cache, value_cache, quant_params, head_mapping, scale,   \
      block_tables, context_lens, max_context_len, alibi_slopes);

// NOTE(woosuk): To reduce the compilation time, we omitted block sizes
// 1, 2, 4, 64, 128, 256.
#define CALL_KERNEL_QUANTIZED_LAUNCHER_BLOCK_SIZE(T)                           \
  switch (block_size) {                                                        \
  /* case 1:                         */                                        \
  /*   CALL_KERNEL_QUANTIZED_LAUNCHER(T, 1);   */                              \
  /*   break;                        */                                        \
  /* case 2:                         */                                        \
  /*   CALL_KERNEL_QUANTIZED_LAUNCHER(T, 2);   */                              \
  /*   break;                        */                                        \
  /* case 4:                         */                                        \
  /*   CALL_KERNEL_QUANTIZED_LAUNCHER(T, 4);   */                              \
  /*   break;                        */                                        \
  case 8:                                                                      \
    CALL_KERNEL_QUANTIZED_LAUNCHER(T, 8);                                      \
    break;                                                                     \
  case 16:                                                                     \
    CALL_KERNEL_QUANTIZED_LAUNCHER(T, 16);                                     \
    break;                                                                     \
  case 32:                                                                     \
    CALL_KERNEL_QUANTIZED_LAUNCHER(T, 32);                                     \
    break;                                                                     \
  /* case 64:                        */                                        \
  /*   CALL_KERNEL_QUANTIZED_LAUNCHER(T, 64);  */                              \
  /*   break;                        */                                        \
  /* case 128:                       */                                        \
  /*   CALL_KERNEL_QUANTIZED_LAUNCHER(T, 128); */                              \
  /*   break;                        */                                        \
  /* case 256:                       */                                        \
  /*   CALL_KERNEL_QUANTIZED_LAUNCHER(T, 256); */                              \
  /*   break;                        */                                        \
  default:                                                                     \
...
void single_query_cached_kv_attention_quantized(
    ...
    torch::Tensor &quant_params, // [num_blocks, num_heads, block_size, 4]
  ...
  if (query.dtype() == at::ScalarType::Float) {
    CALL_KERNEL_QUANTIZED_LAUNCHER_BLOCK_SIZE(float);
  } else if (query.dtype() == at::ScalarType::Half) {
    CALL_KERNEL_QUANTIZED_LAUNCHER_BLOCK_SIZE(uint16_t);
  } else if (query.dtype() == at::ScalarType::BFloat16) {
    CALL_KERNEL_QUANTIZED_LAUNCHER_BLOCK_SIZE(__nv_bfloat16);
  } else {
