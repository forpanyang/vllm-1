...
template <typename scalar_t, int HEAD_SIZE, int BLOCK_SIZE,
          int NUM_THREADS>
__global__ void single_query_cached_kv_attention_fp8_kernel(
  ...
  using Quant_k_vec = typename Vec<uint8_t, VEC_SIZE>::Type;
...
#pragma unroll
      for (int j = 0; j < NUM_VECS_PER_THREAD; j++) {
        ...
        Quant_k_vec quant_k_vec = *reinterpret_cast<const Quant_k_vec *>(
            k_ptr + offset1 * BLOCK_SIZE * x + offset2);
        // <fp8, 2> -> <fp16, 2>
        k_vecs[j] = fp8e5m2_to_half<K_vec, Quant_k_vec>(quant_k_vec);
      }
...
#pragma unroll
  ...
  using Quant_v_vec = typename Vec<uint8_t, V_VEC_SIZE>::Type;
...
#pragma unroll
    for (int i = 0; i < NUM_ROWS_PER_THREAD; i++) {
      ...
      if (row_idx < HEAD_SIZE) {
        ...
        Quant_v_vec quant_v_vec =
            *reinterpret_cast<const Quant_v_vec *>(v_ptr + offset);
        // <fp8, 2> -> <fp16, 2>
        V_vec v_vec = fp8e5m2_to_half<V_vec, Quant_v_vec>(quant_v_vec);
...
#define LAUNCH_ATTENTION_KERNEL(T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS)         \
  cudaFuncSetAttribute(vllm::single_query_cached_kv_attention_fp8_kernel<      \
                           T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS>,             \
                       cudaFuncAttributeMaxDynamicSharedMemorySize,            \
                       shared_mem_size);                                       \
  vllm::single_query_cached_kv_attention_fp8_kernel<T, HEAD_SIZE, BLOCK_SIZE,  \
                                                    NUM_THREADS>               \
      <<<grid, block, shared_mem_size, stream>>>(                              \
          out_ptr, query_ptr, key_cache_ptr, value_cache_ptr,                  \
          head_mapping_ptr, scale, block_tables_ptr, context_lens_ptr,         \
          max_num_blocks_per_seq, alibi_slopes_ptr, q_stride, kv_block_stride, \
          kv_head_stride);

// TODO(woosuk): Tune NUM_THREADS.
template <typename T, int BLOCK_SIZE, int NUM_THREADS = 128>
void single_query_cached_kv_attention_fp8_launcher(
...
#define CALL_KERNEL_LAUNCHER(T, BLOCK_SIZE)                                    \
  single_query_cached_kv_attention_fp8_launcher<T, BLOCK_SIZE>(                \
...
void single_query_cached_kv_attention_fp8(
